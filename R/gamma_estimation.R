...
#
# }
# fnc.form.Aj.theta5M <- function(deriv.alphaj.s){
...
#
#
# ### variables needeed in this function (lambda.tilde.mle, v.beta.mle,  )
#
# l.lambdaj<-list()
# for(j in 1:J){
...
# #  gamma.mj
#
# gamma.fn<-function(M, j, data.x, data.y, sigma.sq.mle, beta.mle,l.Omega.inv,l.lambdaj, S11.names, Spro.names)
# {
#   data.xs<-data.x[data.x$idj==idj[j],]
#   n.j<-length(data.xs$idj)
#
#   Z.j<-Zj.var(M,j,data.x, S11.names)
#   Y.j<-Yj.var(M,j,data.y, Spro.names)
#   v.mle.j<-Y.j-Z.j%*%beta.mle
#
#   omega.j.inv<- l.Omega.inv[[j]]
#   a<-t(as.matrix(l.lambdaj[[j]]))%x%t(iota.nj(n.j))
#   gamma.j<- a %*% omega.j.inv %*% v.mle.j
#   return(gamma.j)
# }
#
# lst.gamma<-list()
...
#   n.j <- n[j]
#
#   Z.j <- Zj.var(data.x, idj, M, j)
#   Y.j <- Yj.var(data.y, idj, j)
#
#   v.mle.j <- Y.j - Z.j %*% beta.mle
#
#   omega.j.inv <- l.Omega.inv[[j]]
...
#
# va.out<-cbind(univ.id.nom,gamma)
#
# #-----------------------------------------------------------------
